...
* An AWS Route53 Hosted zone in your account.
  ...
  * There is no need to remove the custom URLs from the [variables.tf](terraform/variables.tf) or [terraform.tfvars](terraform.tfvars) files. References in the demo deploys will break if they are removed, so best to just ignore them. Variables in `terraform.tfvars` will override `variables.tf`.
...
* Setup [basic cluster variables](terraform.tfvars)
  * cluster_name
  * cluster_name_short
  * s3_state_bucket
  * s3_backup_bucket
  * dns_urls
* Setup [advanced cluster variables](terraform.tfvars)
  * instance_types
  * instances
  * kubernetes
  * cluster_tags
